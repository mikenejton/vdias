...
class VerificationItem(models.Model):
    ...
    cronos = models.TextField('Кронос', blank=True, null=True)
    fms_not_ok = models.BooleanField('ФМС', default=False)
    rosfin = models.BooleanField('Росфинмониторинг', default=False)
    fssp = models.TextField('ФССП', blank=True, null=True)
    docs_full = models.BooleanField('Полнота и качество документов', default=False)
    bankruptcy = models.TextField('Сайт по банкротству', blank=True, null=True)
    сourt = models.TextField('Суды', blank=True, null=True)
    contur_focus = models.TextField('Контур-Фокус', blank=True, null=True)
    affiliation = models.TextField('Проверка на аффилированность', blank=True, null=True)
    created = models.DateTimeField('Дата создания', auto_now_add=True)
    author = models.ForeignKey(ExtendedUser, on_delete=models.PROTECT, verbose_name='Автор')
    def save(self, *args, **kwargs):
        if self.fixed:
